...
MySensors_Node_Sensor_Dht22::~MySensors_Node_Sensor_Dht22() {

}

MySensors_Node_Sensor* MySensors_Node_Sensor_Dht22::clone() const {
  return new MySensors_Node_Sensor_Dht22( *this );
}

void MySensors_Node_Sensor_Dht22::node_sensor_present( ) {
  present( get_sensor_id(), S_TEMP, ( get_description() + " Temperature" ).c_str());
  present( _humidity_id, S_HUM, ( get_description() + " Humidity" ).c_str());
}

void MySensors_Node_Sensor_Dht22::node_sensor_setup( ) {
  DEBUG_MSG(F("[MySensors_Node_Sensor_Dht22] Setup.\n"));
}

void MySensors_Node_Sensor_Dht22::node_sensor_loop( ) {
  if ( millis() > _next_read ) { // time to check
    ...
    if( ! dht.read( _pin, &temp, &hum, NULL )) {
        ...
        temp += _offset;
        DEBUG_MSG(F("[MySensors_Node_Sensor_Dht22] Sending Temperature: "));
        DEBUG_MSG(temp);
      ...
      DEBUG_MSG(F("[MySensors_Node_Sensor_Dht22] Bad read from device.\n"));
    }
